...
const oldLogic = code.substring(code.indexOf('const vendorScores = useMemo(() => {'), code.indexOf('});', code.indexOf('const vendorScores = useMemo(() => {')) + 3);

const newLogic = `  const { paymentScores, receiveScores } = useMemo(() => {
...
// Now replace the JSX
const oldJsxStart = code.indexOf('<Card className="overflow-hidden">');
const oldJsxEndStr = '        </Card>\n      )}';
const oldJsxEnd = code.indexOf(oldJsxEndStr, oldJsxStart) + oldJsxEndStr.length;

const oldJsx = code.substring(oldJsxStart, oldJsxEnd);

const newJsx = `<div className="space-y-6">
