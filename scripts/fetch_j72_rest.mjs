...
async function main() {
  ...
  for (const pr of prs) {
    ...
    if (fields.projectId && fields.projectId.stringValue === 'J-72') {
      if (fields.status && fields.status.stringValue === 'Rejected') continue;
      ...
      for (const itemObj of items) {
        ...
        const desc = item.description ? item.description.stringValue : '';
        if (costCode === '007001001') {
          ...
          console.log('PR', pr.name.split('/').pop(), 'Status:', fields.status ? fields.status.stringValue : 'None', 'Amt:', amt, 'Desc:', desc, 'SubItem:', item.subItemId ? item.subItemId.stringValue : 'None');
        }
      }
    }
  }
    ...
    if (fields.projectId && fields.projectId.stringValue === 'J-72') {
      ...
      for (const itemObj of items) {
        ...
        if (costCode === '007001001') {
          ...
          console.log('PO', po.name.split('/').pop(), 'Status:', fields.status ? fields.status.stringValue : 'None', 'Amt:', amt, 'Desc:', desc, 'SubItem:', item.subItemId ? item.subItemId.stringValue : 'None');
        }
      }
    }
  }
